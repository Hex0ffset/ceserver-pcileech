...
class MockWinsockWrapper : public WinsockWrapper {
public:
...
protected:
    std::shared_ptr<MockWinsockWrapper> socketWrapper;

    void SetUp() override {
        socketWrapper = std::reinterpret_pointer_cast<MockWinsockWrapper>(
            MockWinsockWrapper::createSocket(AF_INET, SOCK_STREAM, 0));
    }
...
TEST_F(SocketWrapperTest, SendReceiveData) {
    ...
    auto clientSocket = MockWinsockWrapper::createSocket(AF_INET, SOCK_STREAM, 0);
